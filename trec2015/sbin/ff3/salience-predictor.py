...
events = [e for e in cuttsum.events.get_events()
          if e.query_num < 11 and e.query_num not in set([ 7])]
events = events
y = []
X = []
    ...
    for max_depth in [3]: #, 2, 3, 4, 5]:
        for lr in [1.,]: # 1.,]:
            for est in [100, ]:
                ...
                plt.savefig("{}_hard.png".format(events[i].fs_name()))
                plt.close("all")
                ...
                #plt.legend()
                plt.gcf().suptitle(events[i].fs_name())
                plt.savefig("{}_soft.png".format(events[i].fs_name()))
                plt.close("all")
                
                plt.hist(y_test_s.ravel() - y_pred.ravel(), bins=25)
                
                plt.gcf().suptitle(events[i].fs_name()+" residuals")
                plt.savefig("{}_res.png".format(events[i].fs_name()))
                plt.close("all")
