...
#            ridx = idx
        ...
        return fmap.inv(idx / 2), idx, w
    
    def get_feature_weights(self):
        select_feats = []
        for i in xrange(fmap._idx):
            fname, idx, weight = self.feat_weight(i * 2)
        ...
        next_feats = []
        for i in xrange(fmap._idx):
            fname, idx, weight = self.feat_weight(i * 2 + 1)
...
#                feats["I"].append(("{}^time".format(feat), val * days))
        ...
        if self._keep_scores is True:

            select_weight = sum(self.vw.get_weight(idx * 2 , 0) * val for idx, val in ifeats['a'])
            next_weight = sum(self.vw.get_weight(idx * 2 + 1, 0) * val for idx, val in ifeats['a'])
...
def predict(task, event_stream, n_iter):
    ...
    task._keep_scores = False
    assert len(pred) == len(task._scores)
    pred = ["SELECT" if p == SELECT else "SKIP" for p in pred]
#    scores = task._scores
...
#p_nex, action
    ...
    summary = []
    nuggets = set()
    for action, (_, sent), costs in izip(pred, event_stream[1].iterrows(), task._scores):
        if action == "SELECT":
            assert costs["SELECT"] <= costs["NEXT"]
        else:
            assert costs["SELECT"] >= costs["NEXT"]
        gain = len(sent["nuggets"] - nuggets)
        if action == "SELECT": 
...
#                "timestamp": sent["timestamp"],
                "action": action,
                "select_cost": costs["SELECT"],
                "skip_cost": costs["NEXT"],
                "gain": gain, 
#                "nuggets": ",".join(sent["nuggets"]), 
                "update text": sent["pretty text"]
            })
            ...
            summary.append({"action": action, 
                ...
                "select_cost": costs["SELECT"],
                "skip_cost": costs["NEXT"],
                "gain": gain, "update text": sent['pretty text']})
        if gain > 0:
            oracle = "SELECT"
        else:
            oracle = "SKIP"
    ...
    if size_y + size_y_hat == 0:
        ...
        loss = 1 - 2 * float(y_int_y_hat) / (size_y + size_y_hat)
    ...
    return egain, comp, f1, loss, pd.DataFrame(summary)
...
def do_work(train_instances, dev_instances, test_instances, sample_size, samples_per_event,
    ...
    for n_iter in xrange(1, iters + 1):
        ...
        for i, inst in enumerate(train_instances):
            egain, comp, f1, loss, summaries = predict(task, inst, n_iter)
            print summaries
            print egain, comp, f1, loss
            all_train_scores.append({"iter": n_iter, "Train E[gain]": egain, "Train Comp.": comp, "Train F1": f1, "Train Loss": loss})
        ...
        df_trn = pd.DataFrame(all_train_scores)
        df_u_trn = df_trn.groupby("iter").mean().reset_index(drop=True)
        print df_u    
        print df_u_trn
        print pd.concat([df_u, df_u_trn], axis=1)
...
def main(sample_size, samples_per_event, gold_probs, iters, l2, log_time,
    ...
    streams = []
    for event in cuttsum.events.get_events()[:2]:
