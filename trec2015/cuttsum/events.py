...
def get_events(by_query_ids=None, by_event_types=None):
    events_2013 = get_2013_events(by_query_ids=by_query_ids, 
        ...
        if elem.tag == 'event':
            ...
            if by_query_ids is not None:
                if query_id in by_query_ids:
                    ts_events.append(
                        Event(
                            query_id, title, event_type, query, start, end, 
                            wiki))
                ...
                ts_events.append(
                    Event(
                        query_id, title, event_type, query, start, end, wiki))
